...
| Admin   | admin@medilink.com       | pass123  |

## Backend Contracts Added

The frontend now expects these backend endpoints for verification, admin control, and avatar upload:

- `POST /api/auth/request-verification` with `{ email }`
...
Profile avatar upload is sent as multipart form data with field name `avatar`. In the backend profile update controller, use multer and persist the uploaded path:
