      ...
      {open&&(
        <div className={styles.chatPanel}>
          <div className={styles.chatHeader}>
            ...
            <div className={styles.chatSub}>Health Assistant · Always available</div>
          </div>
          <div className={styles.chatMessages}>
            {msgs.map(m=>(
              <div
                className={`${styles.messageWrap} ${
                  m.role === 'user' ? styles.messageWrapUser : styles.messageWrapBot
                }`}
                key={m.id}
              >
                <div
          ...
          <div className={styles.suggestions}>
            {SUGGESTIONS.slice(0,2).map(s=>(
              <button className={styles.suggestionBtn} key={s} onClick={()=>send(s)} type="button">{s}</button>
            ))}
          </div>
          <div className={styles.inputRow}>
            ...
            />
            <button className={styles.sendBtn} onClick={()=>send()} type="button">Send</button>
          </div>
        </div>
      )}
    </>
