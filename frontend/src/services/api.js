...
export const auth = {
  login: async (email, password) => {
    ...
    if (d.data?.requiresVerification || d.data?.user?.isVerified === false || d.data?.user?.emailVerified === false) {
      const err = new Error(d.message || 'Please verify your email before login');
  ...
  getMe: () => req('/auth/me'),
  requestVerification: (email) => req('/auth/request-verification', { method: 'POST', body: JSON.stringify({ email }) }),
  verifyEmail: (email, code) => req('/auth/verify-email', { method: 'POST', body: JSON.stringify({ email, code }) }),
...
export const doctors = {
  ...
  updateProfile: (d) => req('/doctors/profile', { method: 'PUT', body: typeof FormData !== 'undefined' && d instanceof FormData ? d : JSON.stringify(d) }),
...
export const patients = {
  ...
  updateProfile: (d) => req('/patients/profile', { method: 'PUT', body: typeof FormData !== 'undefined' && d instanceof FormData ? d : JSON.stringify(d) }),
...
export const admin = {
  ...
  verifyUser: (id, verified = true) => req(`/admin/users/${id}`, { method: 'PUT', body: JSON.stringify({ isEmailVerified: verified }) }),
  updateUserStatus: (id, status) => status === 'active'
...
export const orders = {
  create: (payload) => req('/orders', { method: 'POST', body: JSON.stringify(payload) }),
  getAll: (p = {}) => req(`/orders?${new URLSearchParams(p)}`),
  getById: (id) => req(`/orders/${id}`),
  cancel: (id, reason) => req(`/orders/${id}/cancel`, { method: 'PUT', body: JSON.stringify({ reason }) }),
};
