...
import { connectSocket, disconnectSocket, getSocket, EVENTS } from '../services/socket';
...
export function AppProvider({ children }) {
  ...
  const onSocketConnectError = useCallback((error) => {
    if (!error) return;
  ...
  useEffect(() => {
    ...
    const pushNotification = ({ title, body, type = 'general', page, params }) => {
      const item = {
    ...
    const onMessage = (msg = {}) => {
      ...
      const senderName = msg.sender?.name || (msg.senderRole === 'doctor' ? 'Doctor' : 'Patient');
      pushNotification({
    ...
    const onGenericNotification = (payload = {}) => {
      const consultationId = payload.consultationId || payload.consultation?._id;
      pushNotification({
        ...
        params: payload.params || (consultationId ? { consultationId } : undefined),
      });
    };
    ...
    const onConsultationRequest = (payload = {}) => {
      ...
      if (user?.role === 'doctor' && payload.consultationId) {
        if (activePage !== PAGES.DOCTOR_DASHBOARD) {
          navigate(PAGES.CONSULTATION, { consultationId: payload.consultationId });
        }
      }
    };
  ...
  return (
    <Ctx.Provider value={{
      ...
      selectedDoctor, selectedPrescriptionId, selectedConsultationId,
      notifications, notificationItems, notificationPermission, toast,
      login, logout, navigate, showToast, syncSession, setNotifications,
      requestNotificationPermission, clearNotifications, dismissNotification,
